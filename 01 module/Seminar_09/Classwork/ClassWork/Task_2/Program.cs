    ...
    class Program
    {
        ...
        static void GetInformationAboutFile(string dir = "")
        {
            ...
            if (File.Exists(dir + "\\" + s))  // Проверка на существование файла
            {
                ...
                //Console.WriteLine($"Свойства доступа: {File.GetA(dir + "\\" + s)}"); // Свойства доступа
...
Для того, чтобы выйти из программы используйте команду exit");
                Console.WriteLine("Для того чтобы ввести в путь папку, содержащую пробелы экрнируйте их");
                string command = "";
                do
                {
